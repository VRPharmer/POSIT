...
import csv
from preprocessing import stretch_col
...
def PPR(data):
    w = personalized_page_rank(edge_index=data.edge_index, alpha=0.3)
    data.w = stretch_col(w, 132)
    return data
...
class MolDataset(InMemoryDataset):
    ...
    def process(self):
        ...
        target = df[self.tasks].values  # (n_graph, n_tasks)
        smilesList = df.smiles.values
        data_list = []
...
def load_dataset_all(path, dataset, seed, task_type=None, tasks=None, logger=None):
    # save_path = path + 'processed/all_{}_seed_{}.ckpt'.format(dataset, seed)
    ...
    # save_path = '{}.pt'.format(dataset)
    # if not os.path.isfile(save_path):
    #     torch.save(pyg_dataset, save_path)
    # return load_dataset_all(path, dataset, seed, task_type, tasks, logger)
    return pyg_dataset
...
def load_dataset_scaffold(path, dataset, seed, task_type, tasks=None, logger=None):
    ...
    trn_id, val_id, test_id = scaffold_split(pyg_dataset, task_type=task_type, tasks=tasks,
                                             seed=seed, logger=logger)
    del pyg_dataset.data.smiles
    trn, val, test = pyg_dataset[torch.LongTensor(trn_id)], \
                     pyg_dataset[torch.LongTensor(val_id)], \
...
# ---------------------------------
def build_dataset(args, logger, seed):
    task_name, task_type = get_dataset_tasks(args.dataset_name)

    if args.split_type == 'random':
        train_dataset, valid_dataset, test_dataset = load_dataset_random(args.data_path,
                                                                         args.dataset_name,
                                                                         seed,
                                                                         task_type,
                                                                         task_name,
        ...
        train_dataset, valid_dataset, test_dataset = load_dataset_scaffold(args.data_path,
                                                                           args.dataset_name,
                                                                           seed,
                                                                           task_type,
                                                                           task_name,
        ...
        train_dataset, valid_dataset, test_dataset = load_dataset_noise(args.data_path,
                                                                        args.dataset_name,
                                                                        seed,
                                                                        task_type,
                                                                        task_name,
                                                                        logger)
    elif args.split_type == 'all' or args.split_type is None:
        dataset = load_dataset_all(args.data_path,
                                   args.dataset_name,
                                   args.seed,
                                   task_type,
                                   task_name,
...
def build_loader(args, logger, seed):
    train_dataset, valid_dataset, test_dataset = build_dataset(args, logger, seed)
    train_dataloader = tg.data.DataLoader(train_dataset, batch_size=args.batch_size, shuffle=True)
    valid_dataloader = tg.data.DataLoader(valid_dataset, batch_size=args.batch_size)
    test_dataloader = tg.data.DataLoader(test_dataset, batch_size=args.batch_size)
    # todo: recover
    weights = get_data_weights(train_dataset)
    # weights = train_dataset.weights

    return train_dataloader, valid_dataloader, test_dataloader, weights


def get_data_weights(dataset):
    y = dataset.data.y
    weights = []
    for j in range(y.size(1)):
        neg = (y[:, j] == 0).sum()
        pos = (y[:, j] == 1).sum()
        weights.append([(neg + pos) / neg, (neg + pos) / pos])

    return weights


def get_dataset_tasks(name: str):
    if name == 'tox21':
        tasks = ['NR-AR', 'NR-AR-LBD', 'NR-AhR', 'NR-Aromatase', 'NR-ER', 'NR-ER-LBD', 'NR-PPAR-gamma', 'SR-ARE',
                 'SR-ATAD5', 'SR-HSE', 'SR-MMP', 'SR-p53']
        task_type = 'classification'
    elif name == 'sider':
        tasks = ['SIDER' + str(i) for i in range(1, 28)]
        task_type = 'classification'
    elif name == 'bbbp':
        tasks = ['BBBP']
        task_type = 'classification'
        ...
        raise Exception(f'Unknown dataset name: {name}')
